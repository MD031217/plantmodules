...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* ====================== SETUP MODE (AP режим) ====================== */
        body.setup-mode .top-sticky-wrapper,
        body.setup-mode .site-footer,
        body.setup-mode .user-pill,
        body.setup-mode .navigation {
            display: none !important;
        }
        body.setup-mode .page-wrapper { min-height: 100vh; padding-top: 20px; }
        body.setup-mode .main-content { padding: 20px 15px 40px; }
        body.setup-mode .btn-return-ap { display: none !important; }
        body.setup-mode .manual-section {
        ...
        /* ====================== NORMAL MODE ====================== */
        /* НЕ СКРЫВАЕМ навигацию и профиль — они видны как в первой версии */
...
<body class="theme-dark">
    ...
    <script>
        ...
        function detectMode() {
            ...
            const isSetupMode = hostname === '192.168.10.1' || hostname === '' || hostname === '192.168.4.1';
            if (isSetupMode) {
                document.body.classList.add('setup-mode');
                console.log('[Mode] Setup mode activated');
                return true;
            }
            return false;
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const isSetup = detectMode();

            if (!isSetup) {
                if (!checkAuth()) return;
                loadUserToHeader();  // Загружаем профиль — теперь он ВИДЕН в шапке
            }

            // Кнопка возврата к AP
            const returnApBtn = document.getElementById('returnApBtn');
            const returnApModal = document.getElementById('returnApModal');
            if (returnApBtn && !isSetup) {
            ...
            document.getElementById('confirmReturnApBtn')?.addEventListener('click', function() {
                // Выход из профиля: удаляем токен
                localStorage.removeItem('auth_token');
                returnApModal.classList.remove('active');
...
void setup() {
  ...
  // API маршруты
//  server.on("/wifi-setup", HTTP_GET, handleWiFiSetup);  // новая страница
